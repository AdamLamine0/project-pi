...
@Component({
    ...
    <div class="applications-container animate-fade-in-up">
      ...
      <div class="header-section glass-panel">
        <div class="header-content">
          <button mat-icon-button class="back-btn" routerLink="/community/marketplace/manage">
        ...
        <button mat-raised-button color="accent" class="send-quiz-btn" (click)="sendQuizToTop()" *ngIf="!loading && opportunity">
          <mat-icon>send</mat-icon>
          Envoyer quiz aux Top {{ getRecommendedQuizCount() }}
        </button>
      </div>
            ...
            <div *ngFor="let app of topCandidates; let i = index"
              ...
              <div class="application-content">
                ...
                <div class="cv-section" *ngIf="app.cvUrl">
                  <h4>CV</h4>
                  <button mat-button class="cv-link" (click)="downloadCv(app.cvUrl)">
              ...
              <div class="candidate-actions">
                <button mat-raised-button 
                  *ngIf="!app.quizId && app.status !== 'ACCEPTED'"
                  color="primary"
                  class="action-btn"
                ...
                <button mat-raised-button 
                  *ngIf="app.quizId && app.quizScore !== undefined && app.status !== 'INTERVIEW'"
                  color="accent"
                  class="action-btn"
                  (click)="finaliseApplication(app.id)">
                  <mat-icon>verified</mat-icon>
                  Finaliser
                </button>
                <button mat-button 
  ...
  styles: [`
    ...
    .top-badge mat-icon { font-size: 14px; width: 14px; height: 14px; }
    ...
    .score-badge mat-icon { font-size: 18px; width: 18px; height: 18px; }
...
export class OpportunityApplicationsComponent implements OnInit {
  ...
  loadOpportunityData() {
    ...
    this.marketplaceService.getAllOpportunities(0, 100).subscribe({
      next: (page) => {
        ...
        if (!opportunity) {
          this.snackBar.open('Opportunité non trouvée', 'Fermer', {
      ...
      error: () => {
        this.snackBar.open('Erreur lors du chargement de l\'opportunité', 'Fermer', {
          duration: 3000, panelClass: ['snack-error']
        });
      }
    });
    ...
    this.marketplaceService.getTopCandidates(this.opportunityId).subscribe({
      next: candidates => {
        ...
        this.hydrateCandidateNames(candidates);
      },
      error: () => {
  ...
  sendQuizToTop() {
    ...
    this.marketplaceService.sendQuizToTopCandidates(this.opportunityId, count).subscribe({
      next: (results) => {
        this.snackBar.open(
          `${results.length} quiz envoyés aux meilleurs candidats !`,
          'OK',
      ...
      error: (err) => {
        this.snackBar.open('Erreur lors de l\'envoi des quiz', 'Fermer', {
          duration: 3000, panelClass: ['snack-error']
        });
      }
    });
  ...
  updateApplicationStatus(applicationId: string, status: string) {
    this.marketplaceService.updateApplicationStatus(applicationId, status).subscribe({
      next: (res) => {
        this.snackBar.open(`Candidature ${status === 'ACCEPTED' ? 'acceptée' : 'refusée'} avec succès`, 'OK', {
          duration: 3000, panelClass: ['snack-success']
        });
        this.loadOpportunityData();
      },
