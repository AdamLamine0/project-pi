...
export class ConversationListComponent implements OnInit, OnDestroy {
  ...
  fetchUnreadMessages() {
    this.chatService.getUnreadMessages(this.currentUserId).subscribe(msgs => {
  ...
  private extractPrivateConversations(msgs: ChatMessage[]) {
    // Generate private unread conversations from the unread messages
    // Note: To show *read* conversations we would need an endpoint /api/community/conversations.
    // For now we fulfill the requirement: "Show recent private conversations (from unread messages)"
    const privateMsgs = msgs.filter(m => m.type === 'PRIVATE' || !m.groupId);
    const map = new Map<string, { unreadCount: number, lastMessage: string }>();
    
    privateMsgs.forEach(m => {
      const partnerId = String(m.senderId);
      if (!map.has(partnerId)) {
        map.set(partnerId, { unreadCount: 0, lastMessage: '' });
      }
